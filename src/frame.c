/*
 * frame.c - gzip/zlib/deflate frame implementation
 *
...
unsigned int zultra_update_adler32(unsigned int adler, const unsigned char *buf, size_t len) {
   unsigned int sum2;
   unsigned n;
...
static const unsigned int zultra_frame_Crc32Lookup[4][256] = {
   {
...
/// swap endianess
static inline unsigned int zultra_frame_crc32_swap(unsigned int x) {
...
static unsigned int zultra_frame_crc32_4bytes(const void* data, size_t length, unsigned int previousCrc32) {
   ...
   {
#if __BYTE_ORDER == __BIG_ENDIAN
      unsigned int one = *current++ ^ zultra_frame_crc32_swap(crc);
      crc = zultra_frame_Crc32Lookup[0][one & 0xFF] ^
         zultra_frame_Crc32Lookup[1][(one >> 8) & 0xFF] ^
         zultra_frame_Crc32Lookup[2][(one >> 16) & 0xFF] ^
         zultra_frame_Crc32Lookup[3][(one >> 24) & 0xFF];
#else
      unsigned int one = *current++ ^ crc;
      crc = zultra_frame_Crc32Lookup[0][(one >> 24) & 0xFF] ^
         zultra_frame_Crc32Lookup[1][(one >> 16) & 0xFF] ^
         zultra_frame_Crc32Lookup[2][(one >> 8) & 0xFF] ^
         zultra_frame_Crc32Lookup[3][one & 0xFF];
#endif
   ...
   while (length-- != 0)
      crc = (crc >> 8) ^ zultra_frame_Crc32Lookup[0][(crc & 0xFF) ^ *currentChar++];
...
int zultra_frame_get_header_size(const unsigned int nFlags, const void *ppDictionaryData, const int nDictionarySize) {
   if (nFlags & ZULTRA_FLAG_GZIP_FRAMING)
      ...
      if (ppDictionaryData && nDictionarySize)
         return 2 + 4;
      else
         return 2;
   }
...
int zultra_frame_encode_header(unsigned char *pFrameData, const int nMaxFrameDataSize, const unsigned int nFlags, const void *pDictionaryData, const int nDictionarySize) {
   if (nFlags & ZULTRA_FLAG_GZIP_FRAMING) {
      if (nMaxFrameDataSize >= 10) {
         pFrameData[0] = 0x1f;                              /* ID1, ID2: Magic number */
         pFrameData[1] = 0x8b;
         pFrameData[2] = 0x08;                              /* CM: compression method - deflate */
         pFrameData[3] = 0;                                 /* FLG: flags */

         pFrameData[4] = 0;                                 /* MTIME */
         pFrameData[5] = 0;
         pFrameData[6] = 0;
         pFrameData[7] = 0;

         pFrameData[8] = 2;                                 /* XFL: maximum compression */
         pFrameData[9] = 255;                               /* OS: unknown */

         return 10;
      }
      else {
         return ZULTRA_ENCODE_ERR;
      }

   }
   else if (nFlags & ZULTRA_FLAG_ZLIB_FRAMING) {
      if (nMaxFrameDataSize >= 2) {
         pFrameData[0] = 0x78;                              /* 32K window size (7); deflate decompression method (8) */
         pFrameData[1] = 0xc0;                              /* highest compression level (3) */
         if (pDictionaryData && nDictionarySize)
            pFrameData[1] |= 0x20;
         unsigned short nCheck = pFrameData[1] | (((unsigned short)pFrameData[0]) << 8);
         nCheck = 31 - (nCheck % 31);

         pFrameData[1] |= (nCheck & 0x1f);                  /* checksum */
         ...
         if (pDictionaryData && nDictionarySize) {
            ...
            pFrameData[4] = nDictChecksum >> 8;
            pFrameData[3] = nDictChecksum >> 16;
            pFrameData[2] = nDictChecksum >> 24;
...
zultra_frame_checksum_t zultra_frame_init_checksum(const unsigned int nFlags) {
   if (nFlags & ZULTRA_FLAG_GZIP_FRAMING)
      ...
      return zultra_update_adler32(0, NULL, 0);
   else
      return 0;
}
...
int zultra_frame_encode_footer(unsigned char *pFrameData, const int nMaxFrameDataSize, const zultra_frame_checksum_t nChecksum, long long nOriginalSize, const unsigned int nFlags) {
   if (nFlags & ZULTRA_FLAG_GZIP_FRAMING) {
      if (nMaxFrameDataSize >= 8) {
         /* Little-endian CRC32 */
         pFrameData[0] = nChecksum & 0xff;
         pFrameData[1] = nChecksum >> 8;
         pFrameData[2] = nChecksum >> 16;
         pFrameData[3] = nChecksum >> 24;

         /* Original (decompressed) size mod 2^32 */
         pFrameData[4] = (unsigned char)(nOriginalSize & 0xff);
         pFrameData[5] = (unsigned char)(nOriginalSize >> 8);
         pFrameData[6] = (unsigned char)(nOriginalSize >> 16);
         pFrameData[7] = (unsigned char)(nOriginalSize >> 24);
      ...
      if (nMaxFrameDataSize >= 4) {
         /* Big-endian adler32 */
         pFrameData[3] = nChecksum & 0xff;
         pFrameData[2] = nChecksum >> 8;
         pFrameData[1] = nChecksum >> 16;
         pFrameData[0] = nChecksum >> 24;
